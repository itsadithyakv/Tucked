...
| ss. 8–11 | Live ratios; reduced-ratio windows & floors; never count volunteers/students/RCs; 6+ children ⇒ 2 staff | `staff_shift`, `attendance_event`; `domain/ratios` | Room home, supervisor home | `s8_*`, `s11_*` | 0 (engine) / 1 (live) | ⬜ |
| s. 32 | Daily health observation on arrival, incl. parent-reported symptoms | `care_log(health_observation)` | Sign-in flow | `s32_*` | 1 | ⬜ |
| s. 33.1 | Back-to-sleep under 12 mo; direct visual sleep checks < 24 mo, per-child timestamped; rest ≤ 2 h | `care_log(sleep_check, nap_*)` | Room sleep board | `s33_1_*` | 1 | ⬜ |
...
| s. 36(4) | Accident report with evidence parent received a copy | `accident_report` + acknowledgement | Report flow, family ack | `s36_4_*` | 1 | ⬜ |
| s. 37 | Daily written record: dated entry every operating day, auto-cross-referenced, human-closed | `daily_written_record`; 06:00 `pg_cron` draft | Close-the-day flow | `s37_*` | 1 | ⬜ |
...
| s. 40 | Medication: authorisation with dose + schedule/symptoms; label checklist; designated staff; **blanket items logged** | `medication_authorisation`, `medication_administration` | Med flows | `s40_*` | 1 | ⬜ |
...
| s. 50 | Safe arrival & dismissal: expected-but-absent prompt; release only to authorised persons; restrictions hard-block | `pickup_authorisation`, `pickup_restriction` (SQL-level block) | Sign-in/out | `s50_*` | 1 | ⬜ |
| ss. 53–64 | Staff files: RECE, first aid/CPR, VSC 5-year renewal, offence declarations, health assessments | `credential` ledger | Staff files (free wedge) | `s53_*`+ | 1 | ⬜ |
| s. 72(1) | Children's record: all 11 items; missing = `not_applicable`/`parent_declined`, never blank | `child`, `child_record_item` | Enrolment | `s72_*` | 1 | ⬜ |
| s. 72(3) | Attendance per age group: actual times or "absent"; offline; corrections carry who/when/why; continuous across room moves | `attendance_event` | Room sign-in/out, evacuation screen | `s72_attendance_*` | 1 | ⬜ |
| s. 72(5) | Retention: 3 y post-discharge; financial 6 y (O. Reg. 138/15 s. 27.1); purge = anonymise-after | `retention_clock`; scheduled fn | — | `s72_5_*` | 0 (clocks) / 1 | ⬜ |
| s. 72(6) | Medical officer of health export: items 2, 3, 8, 9 exactly | export fn | Console exports | `s72_6_*` | 1 | ⬜ |
| s. 73 | Enrolment never blocked on an optional consent | `consent` model | Enrolment | `s73_*` | 1 | ⬜ |
| s. 75.1 | Waitlist: no fees; self-serve position without exposing others | waitlist (bundle, off) | — | `s75_1_*` | 2 | ⬜ |
| s. 82(2) | Electronic records: staff & Ministry can always get in; offline on premises | offline queue; break-glass access | Room mode | `s82_2_*` | 1 | ⬜ |
...
| PIPEDA | Minimum collection; consent per purpose; access/deletion workflow, 30-day SLA; Canadian hosting | `consent`, RLS, `ca-central-1` | Console | `pipeda_*` | 0–1 | ⬜ |
