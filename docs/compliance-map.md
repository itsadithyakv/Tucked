...
| s. 32 | Daily health observation on arrival, incl. parent-reported symptoms | `care_log(health_observation)` | Sign-in flow | `s32_*` | 1 | 🔶 log type + payload live; arrival-flow UI pending |
| s. 33.1 | Back-to-sleep under 12 mo; direct visual sleep checks < 24 mo, per-child timestamped; rest ≤ 2 h | `care_log(sleep_check, nap_*)` | Room sleep board | `s33_1_*` | 1 | 🔶 DB gating + console register ✅; room prompts pending |
...
| s. 36(4) | Accident report with evidence parent received a copy | `accident_report` + acknowledgement | Report flow, family ack | `s36_4_*` | 1 | 🔶 report + ack RPC + register ✅; family ack UI pending |
...
| s. 50 | Safe arrival & dismissal: expected-but-absent prompt; release only to authorised persons; restrictions hard-block | `pickup_authorisation`, `pickup_restriction` (SQL-level block) | Sign-in/out | `s50_*` | 1 | 🔶 SQL hard block + engine ✅; room UI pending |
...
| s. 72(3) | Attendance per age group: actual times or "absent"; offline; corrections carry who/when/why; continuous across room moves | `attendance_event` | Room sign-in/out, evacuation screen | `s72_attendance_*` | 1 | 🔶 DB + console ✅; offline room UI pending |
