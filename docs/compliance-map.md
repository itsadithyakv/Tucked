...
| s. 50 | Safe arrival & dismissal: expected-but-absent prompt; release only to authorised persons; restrictions hard-block | `pickup_authorisation`, `pickup_restriction` (SQL-level block) | Sign-in/out | `s50_*` | 1 | 🔶 SQL hard block + release-to flow ✅; safe-arrival prompts pending |
