...
-- The centre's own local day, which is what every derived date column uses.
-- Naming it here rather than saying current_date keeps this suite honest when
-- it runs between UTC midnight and Toronto midnight.
create temporary view today_local as
  select (now() at time zone 'America/Toronto')::date as d;
grant select on today_local to public;

-- ── a day with nothing on it ────────────────────────────────────────────────
select alike(
  app.dwr_compose('31900000-0000-4000-8000-000000000001', (select d from today_local)),
  to_char((select d from today_local), 'FMDay FMDD FMMonth YYYY') || ' — Record Centre%',
...
select ok(
  app.dwr_compose('31900000-0000-4000-8000-000000000001', (select d from today_local)) not like '%  %',
...
values ('31900000-0000-4000-8000-000000000001', '41900000-0000-4000-8000-000000000002', '33900000-0000-4000-8000-000000000001', (select d from today_local), now() - interval '3 minutes', '41900000-0000-4000-8000-000000000001');

insert into public.attendance_event (centre_id, child_id, room_id, event_type, actual_time, attendance_date, recorded_by) values
  ('31900000-0000-4000-8000-000000000001', '61900000-0000-4000-8000-000000000001', '33900000-0000-4000-8000-000000000001', 'arrive', now() - interval '2 minutes', (select d from today_local), '41900000-0000-4000-8000-000000000002');
insert into public.attendance_event (centre_id, child_id, room_id, event_type, actual_time, attendance_date, recorded_by) values
  ('31900000-0000-4000-8000-000000000001', '61900000-0000-4000-8000-000000000002', null, 'absent', now() - interval '2 minutes', (select d from today_local), '41900000-0000-4000-8000-000000000002');
...
values ('31900000-0000-4000-8000-000000000001', '33900000-0000-4000-8000-000000000001', (select d from today_local),
        now() - interval '122 minutes', now() - interval '2 minutes', 'fine',
...
values ('31900000-0000-4000-8000-000000000001', (select d from today_local), 'snack_pm', 'Apple slices and cheese', 'Yogurt and berries', 'Cheese ran out', '41900000-0000-4000-8000-000000000002');
...
insert into public.care_log (centre_id, child_id, room_id, log_type, logged_at, payload, recorded_by) values
  ('31900000-0000-4000-8000-000000000001', '61900000-0000-4000-8000-000000000001', '33900000-0000-4000-8000-000000000001', 'nap_start', now() - interval '3 minutes', '{}'::jsonb, '41900000-0000-4000-8000-000000000002'),
  ('31900000-0000-4000-8000-000000000001', '61900000-0000-4000-8000-000000000001', '33900000-0000-4000-8000-000000000001', 'sleep_check', now() - interval '2 minutes', '{"breathing_ok":true,"position":"back"}'::jsonb, '41900000-0000-4000-8000-000000000002'),
  ('31900000-0000-4000-8000-000000000001', '61900000-0000-4000-8000-000000000001', '33900000-0000-4000-8000-000000000001', 'sleep_check', now() - interval '1 minute', '{"breathing_ok":true,"position":"back"}'::jsonb, '41900000-0000-4000-8000-000000000002');
...
values ('31900000-0000-4000-8000-000000000001', '61900000-0000-4000-8000-000000000001', now() - interval '2 minutes', (select d from today_local),
...
select is(
  (select count(*) from unnest(string_to_array(
     app.dwr_compose('31900000-0000-4000-8000-000000000001', (select d from today_local)), E'\n')) l
   where l like '• %'),
  1::bigint,
...
select ok(
  app.dwr_compose('31900000-0000-4000-8000-000000000001', (select d from today_local)) like
    ('%• ' || (select ref ->> 'note'
              ...
              where d.centre_id = '31900000-0000-4000-8000-000000000001'
                and d.record_date = (select d from today_local) and d.room_id is null
              limit 1) || '%'),
  's37_the_summary_quotes_the_cross_reference_word_for_word'
);

select ok(
  app.dwr_compose('31900000-0000-4000-8000-000000000001', (select d from today_local)) not like '%uneventful%',
...
select lives_ok(
  $$select public.regenerate_daily_record_draft(
      (select id from public.daily_written_record
       where centre_id = '31900000-0000-4000-8000-000000000001' and record_date = (select d from today_local) and room_id is null),
